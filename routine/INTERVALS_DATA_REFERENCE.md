...
- **The connector is capped at 15 tool calls/day (free-plan quota), shared across everything hitting it that day** — confirmed 2026-08-22, when a same-day 28-day backfill (many parallel sessions) exhausted it after ~7-10 calls and every subsequent call failed with `Daily limit reached` until it reset at 00:00 UTC. A single ordinary day already costs close to this: `activities_list` + `wellness_recent_list` (2) + per activity `activity_detail_get`/`activity_intervals_get`/`activity_streams_get` (3 each) — a normal 1-activity easy day is ~5 calls, but a club/track night with 3-4 files can be 11-14 calls on its own. **Budget deliberately: pull `activities_list` + `wellness_recent_list` first (cheap, always do these), then spend the rest on the day's activities in priority order** (the hardest/most informative piece first) rather than looping activity-by-activity and running out mid-day. If the quota is hit, do NOT fabricate the remaining fields — write what was retrieved (aggregate summary only) and say plainly in `stream_read` that the deep-read battery couldn't run, per the "never estimate a sample" rule. This quota is a strong candidate for why the routine went dark for a month (2026-07-25 to 2026-08-21) with no `LAST_RUN_ERROR.txt` ever written — a quota error may not have been treated as the "irrecoverable pull failure" §1.5 expects, so it likely failed to log anything rather than logging and stopping. If this repeats, consider whether the account needs a paid-tier quota increase.

---

## Dashboard data files the routine writes (in addition to daily_log.json)

**The canonical procedure lives in `.claude/skills/generate-day-data/SKILL.md` — follow it verbatim.** Summary of what it produces (all COMPACT JSON):

### `data/streams/<YYYY-MM-DD>.json` — one per day with ≥1 activity
Per activity: `meta` (from `activity_detail_get`, mapping in the skill), **`intervals`** (from `activity_intervals_get` → `icu_intervals`: Intervals' server-side auto-detected WORK/RECOVERY segments with exact stream indexes — the dashboard's preferred source of workout structure, works with or without watch laps), and `streams` at **FULL resolution — no downsampling** (`t,d,v,hr,cad,alt,grade,watts`; non-Run: `t,hr` only). **NEVER `latlng`/`temp` — public repo.** Add the date to `data/streams/index.json`.

### `data/wellness.json` — one compact row per day
Upsert both yesterday's and today's rows each run (mapping + rounding in the skill).
