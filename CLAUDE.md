...
- **The Intervals.icu connector is on a free plan capped at 15 tool calls/day (resets 00:00 UTC).** This is far below what even one normal day's close+open needs (a single club/track night split across 3-5 activities can need 10-15 calls on its own), so it is not a transient outage — it recurs daily and silently caps how much of the routine's job can complete. It has already caused two multi-week backlogs (2026-07-26 onward, discovered 2026-08-23; and again through 2026-08-29, discovered 2026-08-30). Until the plan is upgraded (https://easy-intervals.de/dashboard), expect the routine to close at most one activity-day (or one multi-activity night) per run, prioritize the oldest unscored day first, and leave the rest for subsequent runs rather than attempting the full backlog at once. Pure rest days (zero Intervals activities) don't consume this quota and can always be closed from wellness data alone.
