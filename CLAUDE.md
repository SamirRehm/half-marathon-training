...
1. **A static web app** (`public/index.html` + `public/app.js`) — four views: **Today** (prescription, readiness, the call, week-so-far, build signals), **Calendar** (score heatmap + weekly rollup rail, slide-over day panel with full per-activity stream analytics), **Analysis** (10 charts), **The Goal** (gap-to-goal, Labor Day gate tracker, pace curve, boom-bust timeline, era table). Pure HTML/JS/Chart.js, no build step. It reads static JSON at runtime: `daily_log.json`, `activities.json`, `athlete.json`, `wellness.json`, and `streams/<date>.json` (via `streams/index.json`).
...
- `public/index.html` — markup + the design system (CSS custom properties) + the methods dialog.
- `public/app.js` — all logic: data load, the analytics library, the four views, the day panel. Client-side analytics: Minetti-based GAP calibrated to Intervals' activity GAP, Pw:Hr decoupling on moving-time halves, EF (m/beat), NP (30 s rolling ⁴-mean), moving-time km splits, HR-zone times, weekly intensity mix + easy share, uninterrupted-week streak, session classification. Definitions live in the in-app "methods" dialog — keep code and dialog in sync.
- **Charting rules the app follows** (from the `dataviz` skill; don't regress these): never two y-axes — two measures of different scale become two charts; series colours are steps from a documented palette assigned per entity in fixed order, never cycled, and were validated against this page's own surface (`scripts/validate_palette.js`, in-page browser hook since Node isn't installed here); legend whenever >1 series and never for exactly 1; status colours are reserved and always paired with a glyph + word; every chart carries a `data` button revealing its table.
- **Session classification** (`classify()` in app.js) gates on Intervals' calibrated `intensity` **before** looking for structure — a run under ~80% of threshold is never a workout however much its autolaps wander on hills — then requires a genuine gear change (≥0.55 m/s spread, fast group ≥10% clear of the slow group). Workouts render a warmup/reps/recoveries/cooldown table and no km splits; easy/steady/long runs render km splits plus easy-purity, decoupling, EF and cadence signals.
