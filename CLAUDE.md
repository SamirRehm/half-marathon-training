...
1. **A static web app** (`public/index.html` (single self-contained file)) — four views: **Today** (prescription, readiness, the call, week-so-far, build signals), **Calendar** (score heatmap + weekly rollup rail, slide-over day panel with full per-activity stream analytics), **Analysis** (10 charts), **The Goal** (gap-to-goal, Labor Day gate tracker, pace curve, boom-bust timeline, era table). Pure HTML/JS/Chart.js, no build step. It reads static JSON at runtime: `daily_log.json`, `activities.json`, `athlete.json`, `wellness.json`, and `streams/<date>.json` (via `streams/index.json`).
...
- **Single file on purpose.** An earlier version split logic into `public/app.js`; both files ship `Cache-Control: max-age=600`, and because element ids changed between deploys a browser holding one old file plus one new one threw on a null reference before the tabs were wired — the whole page went dead. Keep markup, styles and logic in one document so that skew cannot happen.
