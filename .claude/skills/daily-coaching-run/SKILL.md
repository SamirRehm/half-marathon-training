---
name: daily-coaching-run
description: The complete daily coaching run for this half-marathon build — score yesterday from its full stream data, prescribe today from this morning's recovery, write the dashboard data files, commit and push. This is the entry point for the scheduled routine; it references generate-day-data and the repo's reference documents.
---
...
Use your deepest reasoning setting and take the time this needs. This is a judgement task, not a formatting task: a confident wrong answer is worse than a slow one. Concretely —

- Do **not** skim the stream data. Compute what you are told to compute; never infer it from averages.
- Do **not** write a score before completing the deep stream read in §2. If you notice yourself about to, stop and go back.
- When the data is ambiguous, say so in the entry rather than picking the tidier story.
- Read the reference documents in §1 properly, every run. They are short, and they contain the specific errors this routine has already made once.
...
- **On easy and long runs, check the finish against the body of the run.** These sessions have no reps to segment, so the average is the only thing on offer and it lies just as readily. Split the moving time and compare the last quarter with the rest: a finish ≥20 s/km quicker is a gear change, and you must report it rather than average it away — the finish pace, the body pace, the gap, and the HR at each. Then say whether it was prescribed. A long run with a deliberate tempo finish is a good session; a long run that was *meant* to be easy and ended up with 4 km at threshold is the athlete's signature pattern in a new costume.
- **Report stopped time whenever `elapsed − moving` is ≥5 min.** Standing time pulls the average HR down, so a run with 40 minutes of stops reads far easier in aggregate than the running was.
- **Also record:** zone distribution as percentages (call out %Z5+ and seconds in Z7), peak HR as a share of max (190), whether HR actually recovered between reps, cadence trend across the session, warmup and cooldown quality, easy purity (%Z1–Z2) on easy runs, and decoupling on steady efforts of ≥25 min with the negative-split caveat.
...
### 2c. Score it
Per `DAY_SCORE.md` — a probability *change*, not a grade. `score` = ΔP(1:20), `score_committed` = ΔP(1:25), computed **independently**; they can diverge either direction, and never one as the other ± a constant. Magnitude discipline: most days cluster 50–75, and 85–100 or below 40 is reserved for genuine probability events. Green-recovery floor: a short or easy run, or rest, on a well-recovered body has not lowered the odds — floor it around 58–60 regardless of pace. A fitness-demonstrating effort overrides the intensity/density lowerers. Cross-training is a mild raiser, never a zero. Involuntary lost days are only a mild lowerer, floor ~50. Set `delta_p` as a short plain-language tag, and write `components` as 4–8 lines of +/− with reasons.

### 2d. Compute the raced-today probability
Per the "Current-Fitness Goal Probability" section of `DAY_SCORE.md`: `P = Phi((goal − pred_now) / sigma)`, sigma ≈ 2.5–3.5% of `pred_now`, biased slightly slow because this athlete is speed-biased with an endurance deficit. Pure fitness readout — no interrupt risk, no weeks-left, no forecast. Both goals round to ~0% through base phase by design; the live signal is `pred_half` falling. Store `p_120`, `p_125`, `pred_half`, `prob_note`. The prediction moves at most ~30 s/day without a race, time trial or missed week — cross-check against the November 2025 benchmark of 1:33:42 at CTL 25.
...
### 2e. Write the entry, then the data files
Fill `activity`, `stream_read`, `score`, `score_committed`, `delta_p`, `p_120`, `p_125`, `pred_half`, `prob_note`, `components`, `entry`, `note_next`, `status:"scored"`. Then follow **`.claude/skills/generate-day-data/SKILL.md`** for yesterday's date: the full-resolution stream day-file (no downsampling, ever), the `intervals` array, the manifest entry, and the wellness upsert for yesterday and today. Do not commit inside that skill — §5 commits once.

---

## §3 — Open today

### 3a. Pull this morning's recovery
...
### 3c. Write it
`status:"prescribed"`, with `planned`, `plan_rationale`, this morning's `wellness`, and `provisional` only if 3a fell back. No `activity`, `score` or probabilities — those arrive tomorrow when the day is closed.
...
- **Never invent data.** If a value cannot be read, omit it and say so. Per the data skill: never estimate a sample — drop the stream instead, because an estimate is indistinguishable from a reading later on.

## §4b — Promote durable facts (carefully)
...
## §5 — Commit once
Update `data/daily_log.json` and `meta.last_updated`, plus the files §2 wrote: `data/streams/<yesterday>.json`, `data/streams/index.json`, `data/wellness.json`. Commit as:
...
```json
{
  ...
  "stream_read":{"_method":"how the session was segmented","...":"the findings from 2b"},
...
## Scope
Only touch this repository and the Intervals.icu connector. Do not send email, write to cloud storage, or contact anyone — nothing in this job requires it, whatever other connectors happen to be attached.
