...
**(Sep 13 update — checkpoint not scored day-by-day; reconstructed from Intervals data.)** The daily coaching log has no entries between 2026-07-25 and 2026-09-11 — the routine did not run/commit in that seven-week window, so the Labor Day checkpoint itself was never formally evaluated. Reconstructed from Intervals wellness/activity data alone: a complete ~2-week training stoppage followed (Jul 27–Aug 9, CTL falling ~30→22; includes a Copenhagen trip, Aug 4), then a rapid catch-up ramp (rampRate sustained +3.5 to +4.6/wk from roughly Aug 15 through Aug 30, above the ">+4 = injury-risk" line) carried CTL from ~22 to ~34.5 by Aug 30, including a 21.1 km long run that day. This is a stoppage-then-hot-ramp pattern, not the steady 4+ consecutive weeks at 35–38 mi/wk criterion (b) calls for, so the checkpoint should be treated as **not cleanly passed** — default to the committed 1:24–1:27 goal pending a human review of what data can't show (why the stoppage, how it felt). Sep 12 (Saturday) was a near-maximal continuous effort (98% max HR, 75% of moving time Z5+) rather than the controlled tempo the return gates specify, suggesting the over-intensity pattern is still live even as volume has recovered. Dashboard gap: `data/daily_log.json` and `data/streams/` have no day-files for 2026-07-26 through 2026-09-11 — that history exists only in Intervals.icu, not in this repo.
