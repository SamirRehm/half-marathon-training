...
- **2026:** choppy. Decent Feb (16 km long run, 4:07/km tempo). Mar 22 – May 2: near-zero (6-week gap). Rebuilt May–Jun to ~10–15 mi/wk. CTL ~19, ATL ~27 (form ≈ −8) as of the Jul 4 handoff; a 51-day unscored gap (Jul 26 – Sep 14, still pending backfill, see `data/daily_log.json` meta `known_gaps`) carried CTL to a peak of 37.4 on Sep 6. **Sep 14–19 interruption, resolved:** last run Sep 13; six straight zero-training days, with HRV swinging 92→81→66→59→110→88 and resting HR climbing to 50 against a 46–48 floor, and daily steps collapsing to 1,859 then 13 against a ~6,000–7,500 baseline — read as illness, not training fatigue, per the daily log entries for those dates. CTL fell from the Sep 6 peak of 37.4 to 29.7 over the layoff. Resolution confirmed Sep 19: HRV 88 (back inside baseline), RHR 46 (floor), sleep excellent (quality 1, score 93), steps 8,306 (back above baseline) — the first morning every signal agreed since the interruption began. Restart Sep 20 at deliberately low volume (easy 20–30 min, HR ceiling 145), not resuming from where Sep 13 left off; Sat-tempo gates (§6) reset to require 4+ consecutive clean weeks from the restart, not counting anything before the illness.
