...
- **Intervals.icu connector is on a free plan with a tight daily call quota (roughly 15 calls/day observed).** A full day's close-yesterday + open-today job (wellness pulls, activity list, then detail + intervals + streams per activity) can exceed that on a normal multi-activity day, and cannot come close to covering a backlog of several unscored days in one run. This caused a real outage: 2026-07-26 through 2026-08-22 (29 days) has no daily-log entries or stream files — training continued through the gap (CTL rose 27.8→30.4) but nothing was scored. Do not attempt to close more than a couple of days' backlog in a single run on this plan; a plan upgrade is the real fix. When a backlog exists, prioritize the most recent unscored day(s) over strict chronological order — the Labor Day checkpoint and any live coaching decision need current state, not archaeology.
