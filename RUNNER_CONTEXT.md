...
**(2026-08-31 data-status note, pending full backfill — see below.)** The daily log went dark for 5 weeks (routine outage, not an athlete issue — see §7/`routine/LAST_RUN_ERROR.txt`); 2026-07-26 through 2026-08-30 are not yet scored day-by-day. From verified (non-stream) wellness numbers already pulled: CTL climbed ~22.0 (Aug 10) → ~34.5 (Aug 30), with rampRate reading ≥+3.2/wk continuously since Aug 16 (mostly ≥+4/wk) — a sustained hot ramp by criterion (b)-adjacent risk, not the "healthy build" pace this plan calls for. Separately, Intervals shows an apparent ~2-week gap in real training, Jul 27–Aug 9 (one short walk logged, in Copenhagen, on Aug 4) — likely travel, unconfirmed pending the deep-stream backfill and athlete review. If confirmed, that gap alone breaks checkpoint criterion (a) (8+ consecutive weeks, no interruption) for the Sep 5–7 window. Treat the checkpoint's criterion-(a) read as provisional until the backfill closes this gap and the athlete has reviewed it.
