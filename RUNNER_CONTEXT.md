...
- **2026:** choppy. Decent Feb (16 km long run, 4:07/km tempo). Mar 22 – May 2: near-zero (6-week gap). Rebuilt May–Jun to ~10–15 mi/wk. CTL ~19, ATL ~27 (form ≈ −8) as of the Jul 4 handoff; a 51-day unscored gap (Jul 26 – Sep 14, still pending backfill, see `data/daily_log.json` meta `known_gaps`) carried CTL to a peak of 37.4 on Sep 6. **Sep 14–19 interruption:** last run Sep 13; six straight zero-training days since, with HRV swinging 92→81→66→59→110 (erratic, not a clean rebound), resting HR climbing to 50 against a 46–48 floor, and daily steps collapsing to 1,859 then 13 against a ~6,000–7,500 baseline — read as illness, not training fatigue, per the daily log entries for those dates. CTL had drifted to 29.7 by Sep 19. **Sep 19–23 resolution and return:** illness read clearly resolved by Sep 19 (HRV 88, RHR 46, sleep excellent). The return then repeated the chronic over-intensity pattern immediately and without a break: Sunday Sep 20's prescribed easy-return run (HR ceiling 145) was instead run as a continuous progression closing at 182bpm/96% max HR; Tuesday Sep 22's club — the sanctioned all-out slot, but only the third day back — was a genuine race effort (6 reps at 3:24–3:42/km, faster than recent 5K pace, 70% of the main set at Z5+); Wednesday Sep 23 added an 11.2km progression to 174bpm/95% max on a morning whose own numbers (RHR 49, soft sleep) called for easing off, not up. Zero easy volume was banked across the entire five-day return. Confirms the §3 pattern persists even against the routine's own explicit downgrade instructions written the day before each session.
