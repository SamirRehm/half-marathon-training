...
- **2026:** choppy. Decent Feb (16 km long run, 4:07/km tempo). Mar 22 – May 2: near-zero (6-week gap). Rebuilt May–Jun to ~10–15 mi/wk. CTL ~19, ATL ~27 (form ≈ −8) as of the Jul 4 handoff; a 51-day unscored gap (Jul 26 – Sep 14, still pending backfill, see `data/daily_log.json` meta `known_gaps`) carried CTL to a peak of 37.4 on Sep 6. **Sep 14–19 interruption:** last run Sep 13; six straight zero-training days since, with HRV swinging 92→81→66→59→110 (erratic, not a clean rebound), resting HR climbing to 50 against a 46–48 floor, and daily steps collapsing to 1,859 then 13 against a ~6,000–7,500 baseline — read as illness, not training fatigue, per the daily log entries for those dates. CTL had drifted to 29.7 by Sep 19. **Resolution confirmed Sep 19:** HRV/RHR/sleep/steps all rebounded together that morning (88 ms / 46 / 9h17m quality 1 / 8,306 steps) — illness, and it cleared. **Restart, Sep 20–22, did not go cleanly:** Sunday's return run ignored its own written re-entry protocol (easy 20–30 min, HR ceiling ~145) and became a monotonic progression to 182 bpm (96% of max, Z7); Tuesday's club night two days later ran 70.6% of its main set at Z5+ (peak 187 bpm, 98% of max) with only one rest day between the two efforts and essentially no easy volume banked either side. CTL kept falling through all of it (37.4 Sep 6 peak → 29.1 by Sep 23). So: the illness is resolved, but the ramp is not yet "back on track" — the open risk is the same over-intensity/no-easy-volume signature documented in §3, now showing up on the restart itself, not recovery status.
