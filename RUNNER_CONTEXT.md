...
**2026-09-09 — checkpoint resolved, retroactively: 1:20 is retired.** The routine went dark 2026-07-26 – 2026-09-07 (Intervals.icu free-plan 15-calls/day quota cap, hit mid-backfill on Aug 23 — see commit `be47aad`; the cap has not been confirmed resolved since), so the Sep 5–7 gate was never evaluated on schedule. Reconstructed on 2026-09-09 from Intervals summary data only (activity/wellness lists, not full stream reads — most of this window remains unscored in `daily_log.json` and should be treated as a documented gap, not silently backfilled): a 20 km long run on Jul 26 was followed by a ~13-day near-zero stretch (Jul 28 – Aug 9, including travel) before training resumed, then a fast rebuild carried CTL from a trough of ~19 (Aug 9) to ~37 by Sep 6, including a 21.1 km long run (Aug 30) and resumed track/interval work (Aug 22, 29, Sep 1, Sep 5). That rebuild is real, but **gate (a) — 8 consecutive weeks with no interruption — is failed outright** by the Aug travel/volume gap, which alone is sufficient per the rule agreed above. Gates (c) 10K TT, (d) threshold pace, and (e) weight are indeterminate from summary data alone (no dedicated TT/race in the window; weight still never logged). One failed gate is enough: **per protocol, 1:20 is retired and the race is committed at 1:24–1:27, zero regret.** This call was made by the routine without the athlete present — stands unless corrected on review.
