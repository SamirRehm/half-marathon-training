...
**(2026-09-14 addendum — checkpoint not evaluated, logging gap found.)** The daily routine did not commit anything between 2026-07-25 and 2026-09-13 (51 days) — training continued throughout (CTL 27.8→34+) but went unlogged and unmonitored, so the Sep 5–7 gate above was never formally checked against live criteria. Root cause: the Intervals.icu connector in use is a third-party proxy ("Easy Intervals"), not Intervals.icu's own MCP integration, and its free plan caps at 15 API calls/day — far below the 20–50+ one full daily run needs, so every run since a partial Aug 23 attempt failed before it could write or commit. Needs a human decision: fix the connector (upgrade the proxy's plan, or reauthorize the official Intervals.icu connector), and separately decide whether to commission a deliberate backfill of the missing window or resume logging forward from 2026-09-13 as done here. Spot-check from Aug 25–Sep 13 activity data: intensity has been chronically high (most runs ≥80% intensity, including a 21.1 km "long run" on Aug 30 at 90.7% intensity) — the signature pattern this checkpoint exists to catch, running unchecked through the gate window.
