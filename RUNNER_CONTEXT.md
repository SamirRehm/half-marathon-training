...
- **2026:** choppy. Decent Feb (16 km long run, 4:07/km tempo). Mar 22 – May 2: near-zero (6-week gap). Rebuilt May–Jun to ~10–15 mi/wk. CTL ~19, ATL ~27 (form ≈ −8) as of the Jul 4 handoff; a 51-day unscored gap (Jul 26 – Sep 14, still pending backfill, see `data/daily_log.json` meta `known_gaps`) carried CTL to a peak of 37.4 on Sep 6. **Sep 14–19 interruption:** last run Sep 13; six straight zero-training days since, with HRV swinging 92→81→66→59→110 (erratic, not a clean rebound), resting HR climbing to 50 against a 46–48 floor, and daily steps collapsing to 1,859 then 13 against a ~6,000–7,500 baseline — read as illness, not training fatigue, per the daily log entries for those dates. CTL had drifted to 29.7 by Sep 19, by which point HRV (88), resting HR (46) and sleep (quality 1, score 93) had all normalized overnight — the illness reads resolved. **Sep 20 return-run overreach:** the first run back was prescribed easy 20–30 min at a 145 bpm ceiling specifically regardless of how green the morning looked; instead it was run as a continuous 8.16 km build with zero recovery, warmup to a 4:04/km finish, HR climbing the whole way to a peak of 183 (96% of max) and 24% of the time at Z5+ — the chronic over-intensity pattern reasserting itself on the first day back from a layoff, not just on a fresh base. Monday Sep 21 was full rest (HRV 99, RHR 47, sleep excellent) with no further damage. CTL 28.8 by Sep 22. Treat the block as restarting from low volume, not resuming where Sep 13 left off, and expect the same negotiate-up pattern on the next return-from-layoff day.
