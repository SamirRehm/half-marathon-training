...
- **2026:** choppy. Decent Feb (16 km long run, 4:07/km tempo). Mar 22 – May 2: near-zero (6-week gap). Rebuilt May–Jun to ~10–15 mi/wk. CTL ~19, ATL ~27 (form ≈ −8) as of the Jul 4 handoff; a 51-day unscored gap (Jul 26 – Sep 14, still pending backfill, see `data/daily_log.json` meta `known_gaps`) carried CTL to a peak of 37.4 on Sep 6. **Sep 14–19 interruption:** last run Sep 13; six straight zero-training days since, with HRV swinging 92→81→66→59→110 (erratic, not a clean rebound), resting HR climbing to 50 against a 46–48 floor, and daily steps collapsing to 1,859 then 13 against a ~6,000–7,500 baseline — read as illness, not training fatigue, per the daily log entries for those dates. CTL had drifted to 29.7 by Sep 19. **Sep 20–23 return: the boom-bust pattern reasserted itself immediately.** Wellness fully normalized by Sep 19 (HRV 88, RHR 46, steps 8,306), but the explicit return-from-illness gate (easy 20–30 min, HR ceiling ~145, no club/long-run) was blown on the very first day back: Sep 20 ran 36:38 continuous, progressing 5:20→4:15/km, HR to 175/183 (96% max). Sep 21 rest reset it, but Sep 22's club night — meant to be a restrained return session — became six ~400m reps accelerating to 3:23/km (faster than recent 5K pace) at up to 187 bpm (98.4% max). Sep 23's prescribed-easy run repeated the exact Sep 20 conversion (11.19 km progressing to 4:29/km, HR to 181). Three of four training days in the return window were significantly harder than prescribed, with recovery numbers (RHR 49–50, HRV sliding 94→89→83→80 over the following four mornings) reflecting the accumulated load rather than the illness. Confirmed pattern: an illness or forced layoff does not reset this athlete's intensity discipline — if anything the return window is now the highest-risk moment for it, and return-from-break prescriptions need a firmer mechanism than a written HR ceiling alone.
