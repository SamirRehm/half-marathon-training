...
- **2026:** choppy. Decent Feb (16 km long run, 4:07/km tempo). Mar 22 – May 2: near-zero (6-week gap). Rebuilt May–Jun to ~10–15 mi/wk. CTL ~19, ATL ~27 (form ≈ −8) as of the Jul 4 handoff; a 51-day unscored gap (Jul 26 – Sep 14, still pending backfill, see `data/daily_log.json` meta `known_gaps`) carried CTL to a peak of 37.4 on Sep 6. **Sep 14–19 interruption:** last run Sep 13; six straight zero-training days since, with HRV swinging 92→81→66→59→110 (erratic, not a clean rebound), resting HR climbing to 50 against a 46–48 floor, and daily steps collapsing to 1,859 then 13 against a ~6,000–7,500 baseline — read as illness, not training fatigue, per the daily log entries for those dates. CTL had drifted to 29.7 by Sep 19. Confirm resolution and restart approach in a future entry before treating the ramp as back on track. **Resolved Sep 19** (HRV/RHR/sleep/steps all normalized together), but **the restart repeated the pre-illness pattern rather than breaking it**: of the first three training days back (Sep 20, 22, 23), two were unprescribed hard continuous progressions (Sep 20 return run, 36:38 to 96% max HR against a prescribed easy 20-30min; Sep 23, 52:06 to 86% avg HR against a prescribed easy 35-40min) bracketing a legitimately hard Tuesday club night (Sep 22, near-max 4-rep set), with zero genuinely easy volume banked anywhere in between. See daily log Sep 20/22/23 for the stream-verified detail. Confirms the boom-bust signature (§3) is not confined to the pre-injury era or to high-volume phases — it reasserted on day one of a return from illness, at low volume, with nothing external forcing it.
