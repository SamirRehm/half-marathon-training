...
**Update (2026-08-14) — another boom-bust cycle, checkpoint criterion (a) now unreachable this cycle.** Jul 25–26 repeated the athlete's documented failure mode in miniature: a self-initiated hard track session (Jul 25, HR 32 bpm over the prescribed ceiling) followed by a long run hot from kilometre one with no easy component (Jul 26, HR ~7 bpm over ceiling the whole way), pushing form through −20 (the fatigue-hole line) on two consecutive days. That was followed by a 15-day complete training gap (Jul 27–Aug 9, one Copenhagen walk excepted — the athlete was traveling for at least part of it) before running resumed Aug 10. CTL fell from a 30.0 peak (Jul 26) to a 21.7 trough (Aug 9), back near the Jul 4 baseline (19); recovery metrics (HRV, RHR, sleep) stayed strong throughout the gap with no illness/overreaching signature, consistent with rest/travel rather than injury. This is now the fifth documented occurrence of the intensity-spike → collapse pattern (2022, 2023, 2024→rupture, Mar–Apr 2026, Jul–Aug 2026), each time following an intensity spike rather than gradual overreach. Practical effect: checkpoint criterion (a) — 8+ consecutive interruption-free weeks by Sep 5–7 — cannot be met this cycle regardless of execution from Aug 10 onward; treat the Sep 5–7 checkpoint as informational rather than a live 1:20 gate this time, and treat the current re-entry (from ~Aug 10) as a second base-re-entry phase using the same nearly-all-easy, 20→28 mi/wk logic as the original Jul 4 plan, not a resumption of the late-July build in progress. Also note: the daily coaching routine itself did not run Jul 25–Aug 13 (Intervals.icu MCP connector free-trial quota exhausted each morning before it could pull data — see `routine/LAST_RUN_ERROR.txt`); this section and `data/daily_log.json` were backfilled 2026-08-14 from real Intervals data, with Aug 10–12's activity-level HR/pace still pending a connector-quota reset.
