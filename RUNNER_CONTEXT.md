...
- **2026:** choppy. Decent Feb (16 km long run, 4:07/km tempo). Mar 22 – May 2: near-zero (6-week gap). Rebuilt May–Jun to ~10–15 mi/wk. Base-phase build Jul 4–25 took CTL 19→28. Jul 27–Aug 9: another complete stoppage (~13 days, a Copenhagen trip), CTL fell back to 21.7. Rebuilt Aug 10 onward: 31.6→35.1→39.3 mi/wk across three weeks, CTL 34.8 as of Sep 4 (form ≈ −13, ramp +3.4) — see the Labor Day checkpoint result in §6 for the current assessment.
...
**Labor Day checkpoint result (evaluated 2026-09-05): fails — race is committed at 1:24–1:27.** (a) fails outright: a complete ~13-day training stoppage, Jul 27–Aug 9 (a Copenhagen trip, not an Achilles issue — no tendon complaint at any point), broke the 8-consecutive-week requirement; the "no interruption" clock restarted Aug 10. (b) partial: volume has climbed 31.6 → 35.1 → 39.3 mi/wk across the three weeks since the restart, clearing 35+ twice, but that's not yet "routine" — three weeks of a rebuild, not an established pattern; long-run length is not the gap (16.1, 19.6, then 21.1 km on Aug 16/23/30 already exceed the 16–18 km target). (c) not met: no all-out 10K test or race exists in the gap — the closest data point is the Aug 30 21.1 km run at a submaximal ~91% intensity, not a time trial. (d) not confirmed: recent quality efforts (e.g. Aug 29's continuous ~3:54/km) sit at the July critical-speed estimate, not clearly faster than it, and none of the gap's sessions has had the deep stream read that would confirm a genuine threshold shift. (e) unassessable on weight: logging never started (still null throughout); the HRV/RHR half of that criterion is fine (RHR 43–46, HRV at/above the ~87 baseline). Missing (a) alone is decisive per the gate's own rule — one miss commits the race. This matches the plan's stated honest prior (checkpoint passes ~1 in 5) and changes nothing about the day-to-day plan: keep building, the Saturday-tempo gate is unaffected by this result and is evaluated on its own separate criteria above.

**Data-gap note (2026-07-26 to 2026-09-03):** the daily coaching log went silent for six weeks — no scored entries, no stream files — while the athlete kept training normally per Intervals.icu (see the checkpoint result above for what happened in that window). `data/wellness.json` has been backfilled from Intervals for continuity (real pulled values, not estimates), but the day-by-day scored entries and full-resolution stream files for those six weeks do not exist and were not reconstructed as part of resuming the routine on 2026-09-05 — deep-reading ~30 run-days properly is a deliberate, separate task, not something to rush through under a daily-run budget. Whether to backfill it (and at what priority) is for the athlete to decide.
...
1. **Pull the data.** From Intervals.icu: `wellness_recent_list` for today (sleep secs/score/quality, HRV, RHR, CTL, ATL, ramp rate, VO2max, weight when present) and `activities_list` for today. Then, **for every run without exception** — not only planned sessions, workouts, or runs that look surprising — pull `activity_detail_get`, `activity_intervals_get` and `activity_streams_get`, and **analyse the full second-by-second streams rather than the aggregate averages.** How did pace, HR, cadence and power actually evolve: pacing discipline against the plan, HR drift at steady pace, the finish against the body of the run, surges and hills, cadence stability, stopped time, anything anomalous.

   The mandatory battery is **step 7A of `.claude/skills/generate-day-data/SKILL.md`**; follow it and record the findings in the day's `stream_read`. An average is a claim that a run had one character, and most runs don't — the worst misreads in this log were all on runs whose summaries looked unremarkable (a "5:24 avg" easy run that was an over-fast start plus 40 min of one-way HR drift; a club night that was a 5K raced in six pieces; a "141 bpm easy" long run with 4 km of tempo in the tail). Ask the athlete what data can't tell you — how it felt, strength done, life context, and above all what the session was *meant* to be.
