...
- **2026:** choppy. Decent Feb (16 km long run, 4:07/km tempo). Mar 22 – May 2: near-zero (6-week gap). Rebuilt May–Jun to ~10–15 mi/wk. Jul 4 CTL ~19, building through Jul 26 (CTL 30.0, best of the build — a 20 km flat-pace long run that day, decoupling ~6%, unchanged from the Jul 5 baseline despite ~2x the distance). **Jul 27 – Aug 9: a second life-schedule-crunch gap (~2 weeks, near-zero running, incl. travel) — CTL fell 30.0 → 21.7.** Running resumed Aug 10; by Aug 19 CTL ~25.8, rebuilding. This is the second such gap this build (see Mar–May) and the pattern §3 already names as the real consistency risk — not motivation, not the body, but the life-schedule.
...
**Update (as of Aug 19, ~2.5 weeks out):** the Jul 27 – Aug 9 gap (see §5) makes criterion (b) — 35–38 mi/wk with 16–18 km long runs *routine* by Sep 5–7 — arithmetically very unlikely: CTL fell from 30.0 to 21.7 and had only ~10 days to rebuild before the checkpoint. Criterion (a) is narrowly still intact (no Achilles interruption, specifically), but the spirit of the gate — a clean, uninterrupted volume block — was broken by this second life-schedule gap of the build. Absent a 10K TT/race result and a full audit of the last two weeks' training against (c)–(e), treat the checkpoint as very likely to miss on volume/consistency grounds; the honest prior (already ~1 in 5) is now lower. This does not change the committed 1:24–1:27 target, which remains fully realistic.
