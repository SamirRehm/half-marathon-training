...
**Checkpoint result (assessed 2026-09-07, from Intervals data — see gap note below): FAILED on (b).** Volume was near-zero for roughly two weeks (~Jul 26–Aug 9: CTL fell 28→~22 on minimal running) before a strong rebuild from Aug 10, so "35–38 mi/wk routine for 8 consecutive weeks" cannot hold regardless of current numbers — the interruption alone settles it; (c)/(d)/(e) were not separately checked once (b) failed. Current state as of Sep 6–7 is strong on the numbers that matter for 1:24–1:27: CTL climbed 21.7→37.4 in the four weeks since Aug 9 (rampRate holding 2.9–4.6/wk — above the healthy +1…+2.5/wk build zone, a real risk signal in its own right, tracked in the Sep 6 log entry), and a same-relative-effort comparison (2026-09-06 PM run, 4:38/km @156bpm/83.9% intensity vs the 2026-07-05 baseline of 5:24/km @156bpm/84%) shows a genuine ~46 s/km aerobic gain over nine weeks. **Per protocol, the race is now committed at 1:24–1:27 with zero regret; 1:20-style framing should not reappear in coaching decisions.**

**Daily-log gap (2026-07-26 – 2026-09-05): the routine did not run for six weeks** despite the athlete training through nearly all of it (a real ~2-week low-volume patch aside). No scored/prescribed entries exist for that window; `data/daily_log.json` and the stream/wellness files jump from 2026-07-25 to 2026-09-06. The Sep 6 entry was reconstructed from Intervals.icu's wellness/activity history rather than day-by-day stream reads, and its prediction move (1:33:30→1:28:30) is a six-week catch-up estimate, not a normal day's move. A full backfill of the gap (deep stream reads for each intervening run) has not been attempted — it's a separate, substantial undertaking and needs its own decision, not a silent default. Whether/how to backfill is open.
